...
| 구성 | 역할 |
| --- | --- |
| Vercel | `apps/web` 정적 프론트 |
| Fly.io | `apps/api` NestJS |
| Neon | PostgreSQL |
| Upstash | Redis (선택) |
| GitHub Actions | 매일 아침 digest 트리거 |

### Vercel (프론트)

Root Directory를 `apps/web`으로 두고 빌드 커맨드는 `pnpm build`(또는 모노레포면 루트에서 filter). 환경 변수:

- `VITE_API_URL` = Fly.io API origin (예: `https://briefly-api.fly.dev`)

SPA 라우트는 `apps/web/vercel.json`의 rewrite를 사용한다.

카카오 Redirect URI와 `WEB_ORIGIN` / `KAKAO_REDIRECT_URI`를 배포 도메인에 맞게 바꾼다. 프론트와 API가 다른 사이트이면 쿠키는 `SameSite=None; Secure`로 나간다.

### Fly.io (백엔드)

`apps/api/Dockerfile`을 사용한다.
...
fly secrets set DATABASE_URL=... DATABASE_SSL=true \
  ...
  KAKAO_REDIRECT_URI=https://<api-host>/api/auth/kakao/callback \
  JWT_SECRET=... TOKEN_ENCRYPTION_KEY=... \
  GEMINI_API_KEY=... INTERNAL_SECRET=... \
  WEB_ORIGIN=https://<vercel-host> NODE_ENV=production
```

### GitHub Actions

워크플로: [`.github/workflows/daily-digest.yml`](.github/workflows/daily-digest.yml)

리포지토리 Secrets:

- `API_URL` — `https://<api-host>`
- `INTERNAL_SECRET` — API의 `INTERNAL_SECRET`과 동일

기본 스케줄은 UTC 22:00 (한국 시간 07:00)이다. `workflow_dispatch`로 수동 실행도 가능하다.
